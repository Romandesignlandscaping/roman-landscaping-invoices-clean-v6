...
const STORAGE_KEY = 'roman-landscaping-clean-v5';
...
function defaultState() {
  ...
  return {
    ...
    settings: {
      ...
      promoImage: '/assets/promo-default-v6.png',
      headerLogo: '/assets/header-logo-v6.png',
      defaultDueDays: 14,
      compactView: false,
...
async function warmImages() {
  const paths = ['/assets/header-logo-v6.png', '/assets/promo-default-v6.png'];
    ...
    <div class="app-shell">
      <header class="topbar">
        <div class="brand-wrap">
          <div class="brand-side"><img src="/assets/header-logo-v6.png" alt="Roman logo"></div>
  ...
  return `
    ...
    <section class="grid-2">
      ...
      <div class="section-card">
        ...
        <div class="preview-box"><img id="promoPreview" src="${state.settings.promoImage || '/assets/promo-default-v6.png'}" alt="Promo preview"></div>
...
function createInvoiceSnapshot(invoice) {
  ...
  return {
    ...
    promoImage: state.settings.promoImage || '/assets/promo-default-v6.png',
    headerLogo: state.settings.headerLogo || '/assets/header-logo-v6.png',
...
async function generateInvoicePdf(invoice, { download = true, doc = null, addPage = false } = {}) {
  ...
  const headerLogoPath = snap.headerLogo || state.settings.headerLogo || '/assets/header-logo-v6.png';
  const headerLogo = headerLogoPath?.startsWith('data:') ? headerLogoPath : await loadImageAsDataUrl(headerLogoPath);
  const promoPath = snap.promoImage || state.settings.promoImage || '/assets/promo-default-v6.png';
  ...
  const amountX = pageWidth - margin - 44;
  const textWidth = amountX - textX - 78;
  ...
  localDoc.setFontSize(10.5);
  const subtotalLabelX = pageWidth - margin - 278;
  const subtotalValueX = pageWidth - margin - 188;
  const taxLabelX = pageWidth - margin - 138;
  const taxValueX = pageWidth - margin - 106;
  const totalLabelX = pageWidth - margin - 58;
  const totalValueX = amountX;
  localDoc.setFont('helvetica', 'normal');
  localDoc.text('Subtotal:', subtotalLabelX, y);
  localDoc.text(money(subtotal), subtotalValueX, y, { align: 'right' });
  localDoc.text('Tax:', taxLabelX, y);
  localDoc.text(money(tax), taxValueX, y, { align: 'right' });
  localDoc.setFont('helvetica', 'bold');
  localDoc.text('Total:', totalLabelX, y);
  localDoc.text(money(total), totalValueX, y, { align: 'right' });
  y += 18;
